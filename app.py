...
class GameConfig:
    ...
    MAP_WIDTH = 256                   # largeur de la carte (nombre de tuiles)
    MAP_HEIGHT = 256                  # hauteur de la carte (nombre de tuiles)
    ...
    INITIAL_WALL_PROBABILITY = 0.40   # probabilité initiale d'un mur (ajustée pour plus d'espace)
    SMOOTHING_ITERATIONS = 2          # nombre d'itérations de lissage (réduit pour limiter les murs)
    SMOOTHING_THRESHOLD = 4           # seuil de voisins murs pour qu'une tuile devienne mur
...
class Player(Entity):
    def __init__(self, x: float, y: float):
        ...
        self.reload_timer = 0           # rechargement du tir
    ...
    def update(self, is_walkable: Callable[[float, float], bool]) -> None:
        ...
        # mise à jour du rechargement du tir
    ...
    def shoot(self) -> None:
        # réinitialise le temps de rechargement après un tir
        self.reload_timer = GameConfig.RELOAD_TIME

class Bullet(Entity):
    def __init__(self, x: float, y: float, vx: float, vy: float, owner: str):
        # initialisation de la balle avec sa direction, son état actif et son propriétaire ("player" ou "enemy")
    ...
    def update(self, is_walkable: Callable[[float, float], bool]) -> None:
        # mise à jour de la balle : déplacement et vérification de collision avec un mur
...
class Enemy(Entity):
    ...
    def update(self, player: Player, bullets: List[Bullet], is_walkable: Callable[[float, float], bool]) -> None:
        ...
        # comportement spécifique selon le type d'ennemi
        if self.etype == GameConfig.ENEMY_TYPES["SHOOTER"]:
            ...
            if dist < 8 or self.reload_timer <= 0:
                if dist < 16 and not player.shield_active:
                    ...
                    pass
                self.hp = 0  # l'ennemi explose et disparaît

        self.reload_timer -= 1

    def is_alive(self) -> bool:
        # retourne vrai si l'ennemi est toujours en vie
        return self.hp > 0
...
class Game:
    def __init__(self):
        ...
        # lancement de la musique de fond (la piste 0 doit être définie dans les ressources)
        ...
        for _ in range(GameConfig.SMOOTHING_ITERATIONS):
            ...
            for y in range(1, GameConfig.MAP_HEIGHT - 1):
                for x in range(1, GameConfig.MAP_WIDTH - 1):
                    wall_count = 0
                    # compter le nombre de murs autour de la tuile
                    for j in range(-1, 2):
                        for i in range(-1, 2):
                            if i == 0 and j == 0:
                                continue
    ...
    def generate_enemies(self, count: int) -> None:
        # génération d'ennemis sur des tuiles de sol éloignées du joueur
        attempts = 0
    ...
    def is_walkable(self, x: float, y: float) -> bool:
        # vérifie si la position (x, y) est accessible (doit correspondre à un sol)
    ...
    def update(self) -> None:
        ...
        # gestion du tir du joueur (clic gauche)
        ...
        # fin du jeu si le joueur n'a plus de points de vie
        if self.player.hp <= 0:
            pyxel.quit()
    ...
    def draw(self) -> None:
        ...
        # dessin de la carte (affichage uniquement des tuiles visibles)
